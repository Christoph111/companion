...
describe('resolveFontSizes', () => {
	...
	describe('allowShrink=true (shrink to fit)', () => {
		...
		describe('w:144 h:144 (double-size button)', () => {
			...
			test('heuristic thresholds and sizes scale with area and height', () => {
				// pixel size = 100*144/100/1.2 = 120
				const result = resolveFontSizes(w, h, 120, true, 20)
				expect(r3(result)).toEqual([
					120, 119.52, 102.24, 87.84, 61.92, 47.52, 40.32, 34.56, 30.24, 24.48, 20.16, 18.72, 15.84, 14.4,
				])
			})
		})
	})
